...
;	os_file_exists			- Check if a file exists
...
;	disk_read_root_dir
;	disk_write_root_dir
;	disk_reset_floppy		- Check if a file exists
;	disk_convert_l2hts
; ==================================================================
...
os_get_file_list:
	...
	mov ax, 020Eh			; Params for int 13h: read floppy sectors
					; And read 14 of them
...
.show_dir_init:
	...
	mov word di, [cs:.file_list_tmp]   ; Name destination buffer
...
; 	  CX = location in RAM to load file
...
os_load_file:
	...
	mov [cs:.filename_loc], ax	; Store filename location
	mov [cs:.load_position], cx	; And where to load the file!
...
.next_root_entry:
	...
	mov si, [cs:.filename_loc]	   ; DS:SI = location of filename to load
...
.root_problem:
	xor bx, bx			; If file not found or major disk error,
...
.load_file_sector:
	mov ax, word [cs:.cluster]	; Convert sector to logical
	add ax, 31

	call disk_convert_l2hts 	; Make appropriate params for int 13h

	mov bx, [cs:.load_position]
...
.calculate_cluster_cont:
	mov word [cs:.cluster], ax	; Store cluster

	cmp ax, 0FF8h
	jae .end

	add word [cs:.load_position], 512
	jmp .load_file_sector		; Onto next sector!


.end:
	mov bx, [cs:.file_size] 	; Get file size to pass back in BX
...
os_remove_file:
	...
	mov word [cs:.cluster], ax	   ; And save it
...
.more_clusters:
	mov word ax, [cs:.cluster]	   ; Get cluster contents
...
.to_next_root_entry:
	...
	mov word si, [cs:.filename]	   ; Start searching for filename
	mov cx, 11
	rep cmpsb
...
disk_convert_l2hts:
	...
	xor dx, dx
	div word [cs:SecsPerTrack]	   ; Sectors per track
	...
	;mov dx, 0
	xor dx, dx
	div word [cs:Sides]		   ; Floppy sides
	mov dh, dl			; Head/side
	mov ch, al			; Track
...
; ******************************************************************
	mov dl, [cs:bootdev]		   ; Set correct device
; ******************************************************************
