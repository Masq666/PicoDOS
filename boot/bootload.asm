...
load_file_sector:
	...
	mov ax, 0050h			; Set buffer past what we've already read
	mov es, ax
	mov bx, word [pointer]
...
endit:					; We've got the file to load!
	pop ax				; Clean up the stack (AX was pushed earlier)
					; Let's provide kernel with some info
	mov dh, byte [filesystemID]	; Partition type / File system		(01h = FAT12 in this case)
	mov dl, byte [bootdev]		; Provide kernel with boot device info	(00h = floppy)

	jmp 0050h:0000h 		; Jump to entry point of loaded kernel!
...
; STRINGS AND VARIABLES
	...
	filesystemID	db 01h	; FAT12 Partition type
